...
#  type 'self.lambda' # TODO
  type 'self.load', '(filename : String, ?%bool) -> %bool'
  type 'self.local_variables', '() -> Array<Symbol>'
#  type 'self.loop' #TODO
  type 'self.open', '(path: String, mode: ?String, perm: ?String) -> IO or nil'
  type 'self.open', '(path: String, mode: ?Fixnum, perm: ?String) -> IO or nil'
#  type 'self.open', '(path: String, mode : ?String, perm: ?String) {(IO) -> %any)} -> %any' # TODO: returns block value
#  type 'self.open', '(path: String, mode : ?Fixnum, perm: ?String) {(IO) -> %any)} -> %any' # TODO: returns block value
